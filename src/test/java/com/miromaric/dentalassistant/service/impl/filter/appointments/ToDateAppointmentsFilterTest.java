...
public class ToDateAppointmentsFilterTest {
    private ToDateAppointmentsFilter filter;
    ...
    public void testShouldBeRuturnFilteredList() {
        ...
        filter = new ToDateAppointmentsFilter(new Date(timeInMilliseconds), null);
        List<Appointment> filteredAppointments = filter.filter(dummyList);
        Assert.assertEquals(4, filteredAppointments.size());
    ...
    public void testShouldBeReturnUnfilteredList() {
        filter = new ToDateAppointmentsFilter(null, null);
        List<Appointment> filteredAppointments = filter.filter(dummyList);
